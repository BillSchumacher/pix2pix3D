...
class GauGAN3DLoss(Loss):
    ...
    def run_G(self, z, c, batch, swapping_prob, neural_rendering_resolution, update_emas=False, mode='random_z_image_c'):
        ...
        if mode == 'random_z_image_c':
            ws = self.G.mapping(z, batch['pose'], batch, update_emas=update_emas)
            gen_output = self.G.synthesis(ws, batch['pose'], neural_rendering_resolution=neural_rendering_resolution, update_emas=update_emas)
    ...
    def run_D(self, img, c, blur_sigma=0, blur_sigma_raw=0, update_emas=False):
        input_img = {'image': img['image'].clone(), 'image_raw': img['image_raw'].clone()}
        blur_size = np.floor(blur_sigma * 3)
        if blur_size > 0:
            with torch.autograd.profiler.record_function('blur'):
                f = torch.arange(-blur_size, blur_size + 1, device=input_img['image'].device).div(blur_sigma).square().neg().exp2()
                input_img['image'] = upfirdn2d.filter2d(input_img['image'], f / f.sum())
    ...
    def accumulate_gradients(self, phase, batch, gen_z, gen_c, gain, cur_nimg, debug=False):
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws = self.run_G(gen_z, gen_c, batch, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution, mode=generator_mode)
                gen_logits = self.run_D(gen_img, c_render, blur_sigma=blur_sigma)
                training_stats.report('Loss/scores/fake', gen_logits)
                training_stats.report('Loss/signs/fake', gen_logits.sign())
                loss_Gmain = torch.nn.functional.softplus(-gen_logits)
                ...
                if self.D_semantic is not None:
                    ...
                    if self.G.data_type == 'seg':
                        mask_softmax = torch.nn.functional.softmax(gen_img['semantic'], dim=1)
                        mask_softmax_raw = torch.nn.functional.softmax(gen_img['semantic_raw'], dim=1)
                        input_img['image'] = torch.cat([gen_img['image'].detach(), mask_softmax], dim=1) # Detach to avoid backpropagating through the rgb branch.
                        input_img['image_raw'] = torch.cat([gen_img['image_raw'].detach(), mask_softmax_raw], dim=1)
                    else:
                        input_img['image'] = torch.cat([gen_img['image'].detach(), gen_img['semantic']], dim=1) # Detach to avoid backpropagating through the rgb branch.
                        input_img['image_raw'] = torch.cat([gen_img['image_raw'].detach(), gen_img['semantic_raw']], dim=1)
                    gen_logits_semantic = self.run_D_semantic(input_img, c_render, blur_sigma=blur_sigma)
                    training_stats.report('Loss/scores/fake_semantic', gen_logits_semantic)
                    training_stats.report('Loss/signs/fake_semantic', gen_logits_semantic.sign())
                    loss_Gmain += torch.nn.functional.softplus(-gen_logits_semantic) * self.lambda_D_semantic
                ...
                if generator_mode == 'random_z_image_c':
                    ...
                    loss_G_img_reconstruction = loss_G_img_reconstruction * (1 - float(self.only_raw_recons)) + loss_G_img_reconstruction_raw
                    training_stats.report('Loss/G/loss_img_reconstruction', loss_G_img_reconstruction)
                    # print(loss_G_img_reconstruction.shape, loss_Gmain.shape)
                    loss_Gmain = loss_Gmain + loss_G_img_reconstruction.squeeze(-1).squeeze(-1)

                    if 'semantic' in gen_img:
                        if self.G.data_type == 'seg':
                            loss_G_semantic_reconstruction = cross_entropy2d(gen_img['semantic'], real_mask.squeeze(1).long(), weight=self.seg_weight) * (1 - float(self.only_raw_recons))
                            real_mask_raw = F.interpolate(real_mask, size=neural_rendering_resolution, mode='nearest')
                            loss_G_semantic_reconstruction_raw = cross_entropy2d(gen_img['semantic_raw'], real_mask_raw.squeeze(1).long(), weight=self.seg_weight)
                            loss_G_semantic_reconstruction = loss_G_semantic_reconstruction + loss_G_semantic_reconstruction_raw
                        else:
                            real_mask = batch['mask']
                            ...
                            real_mask_raw = F.interpolate(real_mask, size=neural_rendering_resolution, mode='nearest')
                            loss_G_semantic_reconstruction = F.smooth_l1_loss(gen_img['semantic'], real_mask) * self.edge_weight * (1 - float(self.only_raw_recons)) + F.smooth_l1_loss(gen_img['semantic_raw'], real_mask_raw) * self.edge_weight
                        training_stats.report('Loss/G/loss_semantic_reconstruction', loss_G_semantic_reconstruction)
                        loss_Gmain = loss_Gmain + loss_G_semantic_reconstruction.squeeze(-1).squeeze(-1)

                        
                        loss_G_silhouette = 0
                        if self.silhouette_loss and self.G.data_type == 'seg':
                            real_mask_raw = F.interpolate(real_mask, size=neural_rendering_resolution, mode='nearest')
                            loss_G_silhouette = self.calculate_silhouette_loss(gen_img['weight'], real_mask_raw.long())
                            loss_Gmain = loss_Gmain + loss_G_silhouette
                        training_stats.report('Loss/G/loss_silhouette', loss_G_silhouette)

                    else:
                        training_stats.report('Loss/G/loss_semantic_reconstruction', 0)
        ...
        if phase in ['Dmain', 'Dboth']:
            if torch.rand(1) < self.random_c_prob:
                ...
                c_render = batch['pose']
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, _gen_ws = self.run_G(gen_z, gen_c, batch, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution, update_emas=True, mode=generator_mode)
                gen_logits = self.run_D(gen_img, c_render, blur_sigma=blur_sigma, update_emas=True)
                training_stats.report('Loss/scores/fake', gen_logits)
                training_stats.report('Loss/signs/fake', gen_logits.sign())
                loss_Dgen = torch.nn.functional.softplus(gen_logits)
            with torch.autograd.profiler.record_function('Dgen_backward'):
        ...
        if phase in ['D_semanticmain', 'D_semanticboth']:
            ...
            with torch.autograd.profiler.record_function('Dgen_semantic_forward'):
                ...
                if self.G.data_type == 'seg':
                    mask_softmax = torch.nn.functional.softmax(gen_img['semantic'], dim=1)
                    mask_softmax_raw = torch.nn.functional.softmax(gen_img['semantic_raw'], dim=1)
                    input_img['image'] = torch.cat([gen_img['image'], mask_softmax], dim=1) 
                    input_img['image_raw'] = torch.cat([gen_img['image_raw'], mask_softmax_raw], dim=1)
                else:
                    input_img['image'] = torch.cat([gen_img['image'], gen_img['semantic']], dim=1) 
                    input_img['image_raw'] = torch.cat([gen_img['image_raw'], gen_img['semantic_raw']], dim=1)
                gen_logits_semantic = self.run_D_semantic(input_img, c_render, blur_sigma=blur_sigma)

                training_stats.report('Loss/scores/fake_semantic', gen_logits_semantic)
                training_stats.report('Loss/signs/fake_semantic', gen_logits_semantic.sign())
                loss_Dgen_semantic = torch.nn.functional.softplus(gen_logits_semantic)
        ...
        if phase in ['D_semanticmain', 'D_semanticreg', 'D_semanticboth']:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                real_img_tmp_image = real_img['image'].detach().requires_grad_(phase in ['D_semanticreg', 'D_semanticboth'])
                real_img_tmp_image_raw = real_img['image_raw'].detach().requires_grad_(phase in ['D_semanticreg', 'D_semanticboth'])

                if self.G.data_type == 'seg':
                    real_mask = torch.nn.functional.one_hot(batch['mask'].squeeze(1).long(), num_classes=self.G.semantic_channels).permute(0, 3, 1, 2).float()
                    real_mask_raw = filtered_resizing(real_mask, size=neural_rendering_resolution, f=self.resample_filter, filter_mode=self.filter_mode)

                    real_mask_tmp_image = real_mask.detach().requires_grad_(phase in ['D_semanticreg', 'D_semanticboth'])
                    real_mask_tmp_image_raw = real_mask_raw.detach().requires_grad_(phase in ['D_semanticreg', 'D_semanticboth'])
                else:
                    real_mask = batch['mask']
                    real_mask_raw = filtered_resizing(real_mask, size=neural_rendering_resolution, f=self.resample_filter, filter_mode=self.filter_mode)
                ...
                real_img_tmp = {'image': torch.cat([real_img_tmp_image, real_mask_tmp_image], dim=1), 'image_raw': torch.cat([real_img_tmp_image_raw, real_mask_tmp_image_raw], dim=1)}

                real_logits_semantic = self.run_D_semantic(real_img_tmp, real_c, blur_sigma=blur_sigma)
                training_stats.report('Loss/scores/real_semantic', real_logits_semantic)
                training_stats.report('Loss/signs/real_semantic', real_logits_semantic.sign())

                loss_Dreal_semantic = 0
                if phase in ['D_semanticmain', 'D_semanticboth']:
                    loss_Dreal_semantic = torch.nn.functional.softplus(-real_logits_semantic)
                    training_stats.report('Loss/D/loss_semantic', loss_Dgen_semantic + loss_Dreal_semantic)
                ...
                if phase in ['D_semanticreg', 'D_semanticboth']:
                    if self.dual_discrimination:
                        with torch.autograd.profiler.record_function('r1_grads_semantic'), conv2d_gradfix.no_weight_gradients():
                            r1_grads_semantic = torch.autograd.grad(outputs=[real_logits_semantic.sum()], inputs=[real_img_tmp['image'], real_img_tmp['image_raw']], create_graph=True, only_inputs=True)
                            r1_grads_image_semantic = r1_grads_semantic[0]
                            r1_grads_image_raw_semantic = r1_grads_semantic[1]
                        r1_penalty_semantic = r1_grads_image_semantic.square().sum([1,2,3]) + r1_grads_image_raw_semantic.square().sum([1,2,3])
                    else:
                        with torch.autograd.profiler.record_function('r1_grads_semantic'):
                            r1_grads_semantic = torch.autograd.grad(outputs=[real_logits_semantic.sum()], inputs=[real_img_tmp['image']], create_graph=True, only_inputs=True)
                            r1_grads_image_semantic = r1_grads_semantic[0]
                        r1_penalty_semantic = r1_grads_image_semantic.square().sum([1,2,3])
                    loss_Dr1_semantic = r1_penalty_semantic * r1_gamma * 0.5
                    training_stats.report('Loss/r1_penalty_semantic', r1_penalty_semantic)
                    training_stats.report('Loss/D/reg_semantic', loss_Dr1_semantic)
